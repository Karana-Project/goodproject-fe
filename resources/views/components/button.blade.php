...
@php
  ...
  $base = "inline-block font-medium uppercase {$finalPadding} transition {$class}";
  ...
  $colorClasses = match ($color) {
      ...
      'outline'
          => 'bg-transparent text-whitePrimary border border-whitePrimary rounded-xl hover:bg-whitePrimary hover:text-black',
...
<a href="{{ $href }}" class="{{ $base }} {{ $colorClasses }} inline-flex items-center gap-4">
  {{ $text }}
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 24" fill="none" stroke="currentColor" stroke-width="1.5"
    class="w-12 h-6">
